...
export default function SwapScreen() {
  ...
  const flip = () => { setFromToken(toToken); setToToken(fromToken); setAmount(''); };
  const getRate = () => { const k=`${fromToken}-${toToken}`; const r=`${toToken}-${fromToken}`; if(RATES[k])return RATES[k]; if(RATES[r])return 1/RATES[r]; return 1; };
  const rate = getRate();
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.swapBox}>
          ...
          <View style={styles.swapBoxRow}>
            ...
            <TextInput value={amount} onChangeText={setAmount} placeholder="0.00" placeholderTextColor={theme.muted} keyboardType="decimal-pad" style={[styles.swapInput,{color:theme.text}]} />
          </View>
        </Card>
...
const styles = StyleSheet.create({
  ...
  swapBoxLabel: {fontSize:9,letterSpacing:2}, swapBoxBal: {fontSize:9},
  ...
  flipBtn: {width:38,height:38,borderRadius:19,backgroundColor:'#C9A84C',alignItems:'center',justifyContent:'center',shadowColor:'#C9A84C',shadowOffset:{width:0,height:4},shadowOpacity:0.4,shadowRadius:8,elevation:6},
  ...
  infoKey: {fontSize:10}, infoVal: {fontSize:10},
});
