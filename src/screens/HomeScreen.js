...
import { QNTMLogo, Sparkling, SectionTitle } from '../components';
        ...
        <LinearGradient colors={[theme.heroStart, theme.heroEnd]} style={styles.hero}>
          ...
          <Text style={[styles.balance, { color: theme.text }]}>
            {balVisible ? (
              <Text>
                {'$28,'}
                <Text style={{ color: theme.gold }}>261</Text>
                {'.67'}
              </Text>
            ) : '••••••••'}
          </Text>
          ...
          <TouchableOpacity
            onPress={() => setBalVisible(v => !v)}
            style={[styles.visBtn, { borderColor: theme.border }]}
          >
        ...
        <View style={styles.section}>
          ...
          {TOKENS.map((tk, i) => (
            <View key={tk.symbol} style={[styles.assetRow, i < TOKENS.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
              ...
              <Sparkling data={tk.spark} color={tk.color} />
          ...
          {TRANSACTIONS.map((tx, i) => (
            <View key={i} style={[styles.txRow, i < TRANSACTIONS.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
              <View style={[styles.txIcon, {
                backgroundColor: tx.up ? 'rgba(61,139,94,0.12)' : 'rgba(139,61,61,0.12)',
                borderColor: tx.up ? 'rgba(61,139,94,0.25)' : 'rgba(139,61,61,0.25)',
              }]}>
...
const styles = StyleSheet.create({
  ...
  actionLabel: { fontSize: 8, letterSpacing: 1.5 },
  ...
  tokenBal: { fontSize: 10, marginTop: 2 },
  tokenUsd: { fontSize: 12 },
  tokenChange: { fontSize: 9, marginTop: 2 },
  ...
  txLabel: { fontSize: 12 },
  txSub: { fontSize: 10, marginTop: 1, fontFamily: 'monospace' },
  txAmt: { fontSize: 11 },
  txTime: { fontSize: 9, marginTop: 1 },
});
