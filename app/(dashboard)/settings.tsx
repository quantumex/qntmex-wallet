    ...
    <View style={st.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 110 }} showsVerticalScrollIndicator={false}>
        <View style={st.prof}>
          ...
          <Text style={st.profNm}>{userName || "QNTMEX Wanllet"}</Text>
          <Text style={st.profAd}>{addrShort}</Text>
          <View style={st.connBadge}><Text style={st.connTx}>● Connected</Text></View>
        </View>
        <Section title="Account" rows={[{ ic:"👤", lbl:"Account Info", sub:"View wallet details", onPress:()=>router.push("/(dashboard)/account-info") }]} />
        <Section title="Appearance" rows={[{ ic:"🌙", lbl:"Dark Mode", sub:theme==="dark"?"Enabled":"Disabled", right:<Toggle value={theme==="dark"} onToggle={()=>setTheme(theme==="dark"?"light":"dark")} /> }]} />
        ...
        <Section title="About" rows={[{ ic:"ℹ️", lbl:"Version", right:<Text style={st.versionTx}>1.0.0</Text> },{ ic:"📄", lbl:"Terms of Service" },{ ic:"🛠️", lbl:"Privacy Policy" }]} />
        <View style={st.setSec}><Pressable style={st.btnRed} onPress={()=>setShowClear(true)}><Text style={st.btnRedTx}>Disconnect Wallet</Text></Pressable></View>
      </ScrollView>
      <Modal transparent visible={showClear} animationType="fade">
        <View style={st.overlay}>
          <View style={st.modalBox}>
            ...
            <Text style={st.modalSub}>This will permanently remove all wallet data. Make sure you have backed up your seed phrase.</Text>
            <View style={{flexDirection:"row",gap:12,marginTop:20}}>
              <Pressable style={[st.modalBtn,st.modalBtnOut]} onPress={()=>setShowClear(false)}><Text style={st.modalBtnOutTx}>Keep It</Text></Pressable>
              <Pressable style={[st.modalBtn,st.btnGold]} onPress={handleClear}><Text style={st.btnGoldTx}>Clear</Text></Pressable>
            </View>
          </View>
        </View>
      </Modal>
...
const st = StyleSheet.create({
  ...
  btnGold:{ backgroundColor:C.gold,elevation:6},
