...
export default function SwapSuccess() {
  ...
  if (!txHash) return (<View style={[st.container,{justifyContent:"center",alignItems:"center"}]}><Text style={{color:C.red}}>Invalid hash</Text></View>);
  return (
    <View style={st.container}>
      <View style={st.topBar}><Pressable style={st.closeBtn} onPress={() => router.replace("/(dashboard)/home")}><Text style={{color:C.dim,fontSize:17}}>✕</Text></Pressable></View>
      <View style={st.wrap}>
        <Animated.View style={[st.successIcon,{transform:[{scale:scaleAnim}]}]}><Text style={st.checkmark}>✓</Text></Animated.View>
        <Text style={st.title}>Swap Complete!</Text>
        <Text style={st.sub}>You successfully swapped{"\n"}<Text style={{color:C.gold2,fontWeight:"700"}}>{params.fromAmount} {params.fromSymbol} → {params.toAmount} {params.toSymbol}</Text></Text>
        <View style={st.txCard}>
          <Text style={st.txLbl}>Transaction Hash</Text>
          <View style={st.txRow}><Text style={st.txHash}>{short}</Text><Pressable onPress={async () => { await Clipboard.setStringAsync(txHash); Alert.alert("Copied", "Hash copied"); }}><Text style={{color:C.gold,fontSize:14}}>📋//Text></Pressable></View>
          <Pressable style={st.explorerBtn} onPress={() => Linking.openURL(`https://etherscan.io/tx/${txHash}`).catch(() => {})}><Text style={st.explorerTx}>View on Etherscan ↗</Text></Pressable>
        </View>
        <View style={{width:"100%",gap:10}}>
          <Pressable style={st.btnGold} onPress={() => router.replace("/(dashboard)/home")}><Text style={st.btnGoldTx}>Back to Home</Text></Pressable>
        </View>
      </View>
    </View>
  );
...
const st = StyleSheet.create({
  ...
  txCard:{ width:"100%",backgroundColor:C.card,borderRadius:14,borderWidth:1,borderColor:C.b2,padding:16,marginBottom:28},
  txLbl:{ fontSize:11,fontWeight:"700",color:C.m2},
  ...
  btnGold:{ width:"100%",paddingVertical:15,borderRadius:14,backgroundColor:C.gold,alignItems:"center",elevation:6},
