...
export default function SwapConfirm() {
  ...
  let toDecimals = 18;
  if (params.toSymbol !== "ETH") { const t = TOKENS.find((x) => x.symbol === params.toSymbol); if (t) toDecimals = t.decimals; }
  ...
  const handleConfirm = async () => {
    ...
    if (priceImpact > 15) { Alert.alert("Extreme Price Impact", `${priceImpact.toFixed(2)}% impact. Continue?`, [{ text:"Cancel", style:"cancel" }, { text:"Continue", onPress: () => exec(wallet) }]); return; }
    exec(wallet);
  };
  const exec = async (w: typeof wallet) => {
    ...
    } catch (e: any) { Alert.alert("Swap Failed", e.message || "Try again"); } finally { setIsLoading(false); }
  };
  return (
    <View style={st.container}>
      <View style={st.phdr}><Pressable style={st.bk} onPress={() => router.back()}><Text style={st.bkTx}>←</Text></Pressable><Text style={st.ptitle}>Confirm Swap</Text></View>
      <ScrollView contentContainerStyle={{paddingBottom:100}} showsVerticalScrollIndicator={false}>
        ...
        <View style={[st.detCard,{marginHorizontal:20,marginBottom:14}]}>
          {[["Route","Uniswap V3"],["Slippage",`${slip}%`],["Min. Received",`${minStr} ${params.toSymbol}`],["Network Fee",`~${parseFloat(gasCostETH).toFixed(6)} ETH`],["Price Impact",`${priceImpact.toFixed(2)}%`]].map(([k,v],i,arr) => (
            <View key={k} style={[st.drow,i<arr.length-1&&st.drowBorder]}><Text style={st.dk}>{k}</Text><Text style={[st.dv,k==="Price Impact"&&{color:impactColor}]}>{v}</Text></View>
          ))}
        </View>
        {secs > 0 && (<View style={[st.detCard,{marginHorizontal:20,marginBottom:14}]}><View style={st.drow}><Text style={st.dk}>Quote Expires</Text><Text style={[st.dv,secs<30&&{color:C.red}]}>{fmt()}</Text></View></View>)}
        <View style={st.warn}><Text style={st.warnIc}>⚠️</Text><Text style={st.warnTx}>Output is estimated. You will receive at least the minimum amount or the transaction will revert.</Text></View>
...
const st=StyleSheet.create({container:{flex:1,backgroundColor:C.bg},phdr:{ flexDirection:"row",alignItems:"center",gap:12,paddingHorizontal:20,paddingTop:56,paddingBottom:14},bk:{ width:36,height:36,borderRadius:10,borderWidth:1,borderColor:C.b2,backgroundColor:C.card,alignItems:"center",justifyContent:"center"},bkTx:{color:C.dim,fontSize:17},ptitle:{ fontSize:18,fontWeight:"700",color:C.tx},hero:{alignItems:"center",paddingVertical:24,paddingHorizontal:20},heroLabel:{ fontSize:11,fontWeight:"700",color:C.m2,letterSpacing:1,textTransform:"uppercase",marginBottom:14},fromAmt:{fontSize:42,fontWeight:"200",color:C.tx,letterSpacing:-1.5,lineHeight:50},fromTok:{ fontSize:18,color:C.dim},arrow:{ fontSize:24,color:C.gold,marginVertical:8},toAmt:{ fontSize:42,fontWeight:"200",color:C.gold2,letterSpacing:-1.5,lineHeight:50},toTok;{ fontSize:18,color:C.m2},rateDisp:{ fontSize:12,color:C.dim,marginTop:10},detCard:{ backgroundColor:C.card,borderWidth:1,borderColor:C.b2,borderRadius:14,overflow:"hidden"},drow:{ flexDirection:"row",istifyContent:"space-between",paddingHorizontal:16,paddingVertical:12},drowBorder:{ borderBottomWidth:1,borderBottomColor:C.border},dk:{ fontSize:12,color:C.dim},dv:{ fontSize:12,fontWeight:"500",color:C.tx},warn:{ flexDirection:"row",alignItems:"flex-start",gap:8,borderWidth:1,borderColor:C.gb2,backgroundColor:"rgba(201,168,76,0.03)",borderRadius:12,padding:13,marginHorizontal:20},warnIc:{ fontSize:14},warnTx:{ flex:1,fontSize:11,color:C.m2,lineHeight:17},btnGold:{ width:"100%",paddingVertical:15,borderRadius:14,backgroundColor:C.gold,alignItems:"center",elevation:6},btnGoldTx:{ fontSize:15,fontWeight:"700",color:"#040301"}});
